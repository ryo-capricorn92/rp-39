...
// returns an array of the lengths of each word in the string. Rewrite wordLengths using each
// (written above for your convenience); that is, replace the for loop.
function wordLengths(string) {
 var acc = [];
 var words = string.split(" ");
 	each(words, function(word) { // had issue then I learnd that I way trying to return each and acc which does not work.
...
// which makes this function an ideal use-case of the map abstraction. Using map (also written above for convenience),
// rewrite wordLengths (but write a new function, e.g. wordLengthsMap) again -- but this time using map.
...
function wordsLongerThanThreeOther(string) {
	var words = string.split(" ");
	return filter(words, function(word){
		return (word.length > 3);
			});
}
wordsLongerThanThreeOther("The quick brown fox jumps over the lazy dog.");
